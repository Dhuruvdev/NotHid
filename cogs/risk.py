...
class RiskCog(commands.Cog, name="Risk"):
    ...
    async def scan_prefix(self, ctx: commands.Context, user: discord.Member):
        async with ctx.typing():
            history = storage.get_guild_history(ctx.guild.id)
            result = scoring.calculate_risk(user, history)
            storage.save_scan_result(user.id, ctx.guild.id, result)
            ...
            embed.set_image(url="attachment://analysis_card.png")
            embed.set_footer(text="Cybork — Results are probabilistic, not definitive.")
            await ctx.reply(embed=embed, file=file, view=AnalysisView(result, user), mention_author=False)

    @commands.command(name="risk")
    async def risk_prefix(self, ctx: commands.Context, user: discord.Member):
        async with ctx.typing():
            history = storage.get_guild_history(ctx.guild.id)
            result = scoring.calculate_risk(user, history)
    ...
    @commands.command(name="altsuspect")
    async def altsuspect_prefix(self, ctx: commands.Context, user: discord.Member):
        async with ctx.typing():
            history = storage.get_guild_history(ctx.guild.id)
            result = scoring.calculate_risk(user, history)
    ...
    @commands.command(name="behavior")
    async def behavior_prefix(self, ctx: commands.Context, user: discord.Member):
        async with ctx.typing():
            act = storage.get_user_activity(user.id, ctx.guild.id)
            history = storage.get_guild_history(ctx.guild.id)
            result = scoring.calculate_risk(user, history)
            total_msgs = act["total"]
            channels = act.get("channels", {})
    ...
    async def trustscore_prefix(self, ctx: commands.Context, user: discord.Member):
        async with ctx.typing():
            history = storage.get_guild_history(ctx.guild.id)
            result = scoring.calculate_risk(user, history)
            warnings = storage.get_warnings(user.id, ctx.guild.id)
            flag = storage.get_flag(user.id, ctx.guild.id)
            act = storage.get_user_activity(user.id, ctx.guild.id)
