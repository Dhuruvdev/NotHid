...
class AnalyticsCog(commands.Cog, name="Analytics"):
    ...
    async def topusers(self, interaction: discord.Interaction):
        ...
        top = storage.get_top_users(interaction.guild_id, limit=10)
        embed = discord.Embed(
            title=f"Top Active Members — {interaction.guild.name}",
            color=discord.Color.blurple(),
        )
    ...
    async def inactive(self, interaction: discord.Interaction):
        ...
        for member in interaction.guild.members:
            if member.bot:
                continue
            act = storage.get_user_activity(member.id, interaction.guild_id)
            if act["total"] == 0:
        ...
        if threshold_days:
            embed.description += f"\n*Auto-kick threshold: `{threshold_days}d` (set via /autokick set)*"
    ...
    async def channels(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title=f"Channel Activity — {guild.name}",
            color=discord.Color.blurple(),
        )
